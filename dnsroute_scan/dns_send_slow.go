...
func (p *DNSPoolSlow) send() {
	...
	// [1]	 TOS
	// [2] [3] Total length
	binary.BigEndian.PutUint16(ipv4Hdr[2:4], Ipv4LenSlow)
	// [4] [5] Identification
	...
	binary.Write(udpHdrBuf, binary.BigEndian, uint16(UdpHdrSize+DnsHdrSize+DnsQrySizeSlow)) // length
	...
	binary.Write(dnsHdrBuf, binary.BigEndian, TransactionId)
	...
	// construct DNS Query
	randPfx := GetDomainRandPfx(RandLenSlow)
	preDomain := randPfx + "." + fmt.Sprintf("%02d", p.ttl) + "." + FormatIpv4Slow + "." + QryDomain
	...
	dstIpStrBytes := make([]byte, FormatIpv4LenSlow+1)
	dstIpStrBytes[0] = FormatIpv4LenSlow
...
func (p *DNSPoolSlow) recvDns() {
	...
	for {
		...
		if len(question.Name) != Ipv4TtlDomainLenSlow {
			continue
		}
		p.outDnsTargetChan <- DeformatIpv4(question.Name[2+RandLenSlow+FormatTtlLen:][:FormatIpv4LenSlow])
...
func (p *DNSPoolSlow) recvIcmp() {
	...
	ipv4LenUint8 := uint8(Ipv4LenSlow)
	// 接收ICMP报文
	for {
		buf := make([]byte, 1500)
