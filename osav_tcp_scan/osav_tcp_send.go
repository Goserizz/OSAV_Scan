...
func NewTCPoolv4(remotePort uint16, bufSize int, iface, srcIpStr string, srcMac []byte, dstMac []byte) *TCPoolv4 {
	tcpCks := uint32(0)
...
func (p *TCPoolv4) recv() {
	...
	for {
		...
		if localPort != LOCAL_PORT && flag != (SYN_FLAG | ACK_FLAG) { continue }
